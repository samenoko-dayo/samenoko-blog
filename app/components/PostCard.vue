...
<template>
  ...
  >
    <div
      v-if="post.date"
      class="text-slate-500 text-xs font-bold uppercase tracking-widest mb-4 flex items-center gap-2"
    >
    ...
    <h3
      class="text-xl font-bold text-slate-900 mb-3 group-hover:text-slate-700 transition-colors line-clamp-2"
    >
      {{ post.title }}
    </h3>
    ...
    <p
      ...
      class="text-slate-600 text-sm line-clamp-2 mb-6 leading-relaxed"
    >
      {{ post.description }}
    </p>
    ...
    <div
      ...
      class="mt-auto pt-4 flex flex-wrap gap-2 border-t border-slate-100"
    >
      <span
