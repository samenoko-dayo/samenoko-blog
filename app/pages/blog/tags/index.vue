...
<template>
  <div class="max-w-7xl mx-auto space-y-8">
    ...
    <div v-if="tagsMap.length > 0" class="flex flex-wrap gap-4">
      <NuxtLink v-for="tag in tagsMap" :key="tag.name" :to="`/blog/tags/${tag.name}`"
        ...
        <span
          class="px-2 py-0.5 bg-slate-100 text-slate-500 text-xs font-black rounded-lg group-hover:bg-slate-950 group-hover:text-white transition-all">
          {{ tag.count }}
        </span>
      </NuxtLink>
    </div>
