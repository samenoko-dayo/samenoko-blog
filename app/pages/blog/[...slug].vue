...
onMounted(() => {
  mediumZoom(
    ".article-content img:not(.remark-link-card-plus__image, .remark-link-card-plus__favicon)",
    {
      margin: 16,
...
<template>
  <article v-if="post" class="max-w-7xl mx-auto">
    ...
    <!-- Pagination -->
    <nav
      v-if="surround && (surround.next || surround.prev)"
      class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-12"
    ...
    >
      ...
      >
        ...
        >
          <Icon
            name="lucide:arrow-left-circle"
            class="w-4 h-4 transition-transform group-hover:-translate-x-1"
          />
          次の記事
        </span>
        <span
          class="text-base font-extrabold text-slate-800 leading-snug group-hover:text-slate-950"
